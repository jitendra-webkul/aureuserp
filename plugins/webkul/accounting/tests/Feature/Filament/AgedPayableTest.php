...
use Webkul\Accounting\Filament\Clusters\Reporting\Pages\AgedPayable;
...
beforeEach(function () {
    ...
    FilamentHelper::actingAs(['page_accounting_aged_payable']);

    $this->expense = AccountHelper::account('income');
    $this->expense = AccountHelper::account('expense');
    $this->partner = AccountHelper::partner();
});

it('renders the aged payable page', function () {
    Livewire::test(AgedPayable::class)->assertOk();
});

it('places an invoice that is not yet due in the at-date bucket', function () {
    ReportHelper::postedBillDueOn('2026-03-01', '2026-04-15', 2, 100, $this->expense, $this->partner);

    $data = ReportHelper::data(AgedPayable::class, 'agedPayableData', ['as_of_date' => '2026-03-31']);
...
it('places an invoice overdue by less than one period in the first bucket', function () {
    ReportHelper::postedBillDueOn('2026-03-01', '2026-03-20', 2, 100, $this->expense, $this->partner);

    $data = ReportHelper::data(AgedPayable::class, 'agedPayableData', [
...
it('places an invoice overdue by more than one period in the second bucket', function () {
    ReportHelper::postedBillDueOn('2026-01-01', '2026-02-10', 2, 100, $this->expense, $this->partner);
...
it('totals every bucket for a partner', function () {
    ReportHelper::postedBillDueOn('2026-03-01', '2026-04-15', 1, 100, $this->expense, $this->partner);
    ReportHelper::postedBillDueOn('2026-03-01', '2026-03-20', 1, 50, $this->expense, $this->partner);
...
it('excludes draft invoices from the ageing', function () {
    ReportHelper::draftBill('2026-03-01', 9, 100, $this->expense, $this->partner);
    ReportHelper::postedBillDueOn('2026-03-01', '2026-04-15', 2, 100, $this->expense, $this->partner);
...
it('keeps only the selected partner when filtering', function () {
    ...
    ReportHelper::postedBillDueOn('2026-03-01', '2026-04-15', 2, 100, $this->expense, $this->partner);
    ReportHelper::postedBillDueOn('2026-03-01', '2026-04-15', 5, 100, $this->expense, $other);
