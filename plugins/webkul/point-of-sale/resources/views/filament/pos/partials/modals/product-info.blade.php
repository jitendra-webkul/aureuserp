    ...
    <x-filament::modal id="pos-product-info" width="2xl">
        ...
        <div class="flex flex-col gap-1">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">
                <span class="pos-figure font-semibold text-gray-950 dark:text-white">{{ number_format($info['available'], 2) }}</span>
                {{ __('point-of-sale::filament/pos/pages/terminal.product-info.available') }},
                <span class="pos-figure font-semibold text-gray-950 dark:text-white">{{ number_format($info['forecasted'], 2) }}</span>
            ...
            <div class="flex flex-col gap-2">
                ...
                @foreach ([['price', $info['price']], ['cost', $info['cost']]] as [$key, $value])
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="pos-figure text-gray-950 dark:text-white">{{ $this->money($value) }}</span>
                    </div>
                @endforeach

                <div class="flex items-center justify-between text-sm">
                    <span class="text-gray-500 dark:text-gray-400">{{ __('point-of-sale::filament/pos/pages/terminal.product-info.margin') }}</span>
                    <span class="pos-figure text-gray-950 dark:text-white">
